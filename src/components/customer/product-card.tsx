  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-card border border-line bg-surface shadow-soft-sm transition-transform duration-200 active:scale-[0.99]">
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-3.5">
        <button
          onClick={() => openDetail(product.id)}
          className="clamp-2 min-h-[3.25rem] text-left text-lead font-semibold leading-snug text-ink"
        >
          {product.name}
        </button>
        ...
        <div className="mt-auto flex items-center justify-between gap-2 pt-1.5">
          {benefit ? (
            <span className="clamp-1 max-w-[70%] rounded-pill bg-accent/12 px-2.5 py-1 text-sm font-medium text-accent">
              {benefit}
            </span>
          ) : (
            <span />
